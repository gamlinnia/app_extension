					...
					foreach($val as $k=>$v) {
						...
						if(count(explode($attribute->getData('frontend_label'),$k))>1) {
							...
							if($attribute->getData('frontend_input')=='multiselect') {
								...
								$save_multiselect_count++;

								$saveMultiselect=true;
								...
								foreach($v_arr as $v_item) {
										...
										if(!isset($attributeArray[$v_item])) {
													...
													$option['value']['any_option_name'][0] =$v_item;
													$option['value']['any_option_name'][1] =$v_item;
										...
										print_r('<br/><br/>');
										print_r($attribute->getData('frontend_label').'==='.$attribute->getData('attribute_code').'==='.$v_item.'<br/>');
										print_r($v.'<br/>');
										...
										//$product->setData($attribute->getData('attribute_code'),'2255,2256');
